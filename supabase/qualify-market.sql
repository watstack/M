...
-- Adds 'qualify' (home or away, no draw) to the correlated-legs guard in
-- place_parlay so it can't be combined with match_result, correct_score, or
-- double_chance on the same match.
-- No change needed to selection_wins() — qualify uses exact match like match_result.
